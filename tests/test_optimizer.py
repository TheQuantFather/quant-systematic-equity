...
def _single_sector_industry(n=5):
    """All stocks in one sector / one industry / one cap bucket (no binding cap)."""
    sectors     = ["Tech"]
    industries  = ["Software"]
    ...
    cap_buckets = ["all"]
    B_cap       = np.ones((1, n))
    return sectors, industries, B_sector, B_ind, cap_buckets, B_cap
...
def test_minimize_variance_binding_max_position():
    ...
    L          = np.linalg.cholesky(Sigma)
    b          = np.full(n, 1.0 / n)
    sectors, industries, B_sector, B_ind, cap_buckets, B_cap = _single_sector_industry(n)
