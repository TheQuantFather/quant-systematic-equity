...
def extract_filing_data(
    ...
    if lae is not None and lae != 0:
        ...
        # Equity can be derived if the parsed value is implausible
        if liab is not None:
            computed_equity = lae - liab
            if equity is None or (computed_equity != 0 and abs(equity / computed_equity - 1) > 0.50):
...
def _publish_date(filing) -> str:
    """
    Return the PIT publish date for a Filing object.
    Company.get_filings() objects have acceptance_datetime (precise); index-mode
    Filing objects from get_filings() only have filing_date (same day, good enough).
    """
