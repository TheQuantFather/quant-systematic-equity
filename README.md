...
| **Dashboard** | 9-page Streamlit app — screener, factor deep-dive, backtester, portfolio analytics, risk explorer |
...
| Model | Components |
|-------|-----------|
| Quality | 15 quality factors, equal-weighted |
...
| **Alpha (composite)** | Equal-weight blend of all 7 models |
...
**Q4 derivation**: EDGAR 10-Qs cover Q1–Q3 only. Q4 standalone income/cashflow is derived from the annual 10-K using the accounting identity `Q4 = FY − Q1 − Q2 − Q3`.

**YTD handling**: EDGAR filers that report only cumulative 6M/9M values in XBRL are automatically decomposed to standalone quarters before LTM aggregation.
...
├── create_universe.py            # Build universe.db from iShares N-PORT-P
...
constituents.db   — (security_id, constituent_id, fiscal_year, fiscal_period) · value · publish_date
